...
plot_jac_norm = False
plot_jac_err = False
latent_space_training_video = True
symbols = (0, 1, 2, 3)#(0, 1, 3)
ignored = ()


# DATA
dataset = 'x0-u1-pert-drift-500k.pkl' #'gym-1step-5million.pkl'
num_traj = int(5e5)
traj_len = 3
...
res_bias_fdyn = True
final_act = False
init_ae = nn.init.xavier_normal_ # try kaiming 
init_ae_bias = nn.init.zeros_ #nn.init.zeros_
init_fdyn = nn.init.xavier_normal_ # try kaiming
init_fdyn_bias = nn.init.zeros_
init_fdyn_drift = nn.init.xavier_normal_ #nn.init.xavier_normal_
init_fdyn_offset = nn.init.xavier_normal_
init_fdyn_cntrl = nn.init.xavier_normal_ #nn.init.xavier_normal_
init_fdyn_drift_bias = nn.init.zeros_ #nn.init.normal_ 
init_fdyn_offset_bias = nn.init.zeros_  #nn.init.zeros_
init_fdyn_cntrl_bias = nn.init.zeros_ #nn.init.zeros_
ode = False
ode_method = 'rk4'
...
lr = 1e-4 #5e-4, 1e-3 if mean loss
batch_size = 16384 #512 #16384 #32768
store_model_freq = 5
epochs = 10
test_inc = epochs-1
weight_decay=0.00
